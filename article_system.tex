...
\begin{abstract}
Scavenging energy from the earcanal dynamic motion during jaw motion may be a practical way to enhance the battery autonomy of hearing aids. One of the main challenges is to optimize the extracted energy while coping with the constraints associated with soft human tissues and restricted earcanal volume. We propose a new concept of harvester made of a liquid filled earplug transferring the energy outside the earcanal to a generator. It is composed oa hydraulic amplifier, two hydraulic cylinders actuating a bistable resonator raising the source frequency driving an amplified piezoelectric transducer to generate electricity. The cycling of the resonator is achieved using two innovative flexible hydraulic valves based on the buckling of flexible tubes.\\
A multiphysics coupled model is established in order to determine the system operation requirements and to evaluate its theoretical performance. It exhibits a theoretical energy conversion efficiency of 85\%. The electromechanical performance of the resonator couple to the piezoelectric transducer as well as the hydraulic behaviour of the valves are experimentally investigated. The global model is updated with the experimental data to ameliorate its predictability for a further optimized design. The energy losses are identified, which allow an entire design proposed to improve the experimental energy conversion efficiency to 26\%.
\end{abstract}
...
% Figure \ref{fig:OB_vs_LINEAR} shows in blue the hydraulic behaviour of the ideal flow rate source previously introduced with the upper limit values of pressure and volume variations.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{figure}[!htbp]
% 	\centering
% 	\captionsetup{justification=centering}
